...
/**
 * Buckets a timestamp into its org-local calendar day ("YYYY-MM-DD").
 */
function dayBucket(timestampMs: number, org: Org): string {
  return localDayKey(timestampMs, org.utcOffsetMinutes);
}
...
export function aggregateDaily(events: NormalizedEvent[], org: Org): DailyAggregate {
  ...
  for (const event of events) {
    ...
    const day = dayBucket(event.timestamp, org);
